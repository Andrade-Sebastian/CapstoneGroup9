...
import { serve, Response } from './deps'
...
function onError(err, req, res, next) {
  ...
  res.end((err.length && err) || err.message || code)
}
...
export class Server extends Router {
    ...
    if (this.apps[base] !== void 0)
      throw new Error(
        `Cannot mount ".${method.toLowerCase()}('${lead(
          pattern
        )}')" because a deno-express application at ".use('${base}')" already exists! You should move this handler into your deno-express application instead.`
      )
  ...
  handler(req: any, res:any, info?: any) {
    info = info || parseURL(req)
    let fns = [],
      arr = this.wares,
